...
public class Library {
   ...
   private TransactionDAO transactions;
    ...
    public ArrayList<Transaction> getTransactions() throws SQLException { return transactions.getAll();
    }
    ...
    public Library() throws SQLException, ClassNotFoundException, IllegalAccessException, InstantiationException {
        String url = "jdbc:sqlite:C:\\Users\\aloes\\Desktop\\lib.db";
    ...
    public boolean isAvailable(int bookID) throws SQLException {
        //use the transaction dao getbyBook method
        ArrayList<Transaction> temp = transactions.getByBook(bookID);
        //enhanced for loop to check status
    ...
    public int getBorrowCount(int userID) throws SQLException {
        int count=0;
        //use our getCurrent for transactions dao
        ArrayList<Transaction> temp = transactions.getCurrent();
        //now search for our user id in the current Transactions
        for (Transaction t: temp){
    ...
    public boolean issueBook(int userID, int bookID) throws SQLException {
        //use the dao objects to get the user and book
    ...
    public boolean returnBook(int bookID) throws SQLException {
        ...
        //get book and user object from daos
        User u = users.getUser(userID);
        Book b = books.getBook(bookID);
        ...
        //now update the transaction dao
    ...
    public ArrayList<Book> searchBook(String name) throws SQLException {
        if(name.equals(""))
            return books.getAll();
//use the dao method to search the books
        ArrayList<Book> results = books.getByQuery(name);
        /*
    ...
    public ArrayList<User> searchUser(String name) throws SQLException {
        if(name.equals(""))
            return users.getAll();
        //use the dao method to search the users
        ArrayList<User> results = users.getByQuery(name);
/*
 ...
 */
    ...
    public Book getBook(int bookID) throws SQLException {
        //use the dao getBook method
        Book temp = books.getBook(bookID);
        /*
...
//use the dao  method here
        res = transactions.getCurrent();
        /*
        for(Transaction t: transactions.getAll()){
            if(t.isStatus()){
                res.add(t);
